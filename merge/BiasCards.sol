...
contract BiasCards is Ownable, KIP37, KIP37Burnable, KIP37Pausable {
    using SafeMath for uint256;

    constructor() public KIP37("https://api.dogesound.club/bias-cards/{id}.json") {}

    function uri(uint256 _tokenId) external view returns (string memory) {
        uint256 tokenId = _tokenId;
        if (tokenId == 0) {
            return "https://api.dogesound.club/bias-cards/0.json";
        }

        string memory baseURI = "https://api.dogesound.club/bias-cards/";
        string memory idstr;
